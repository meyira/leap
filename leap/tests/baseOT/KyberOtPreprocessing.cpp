...
int main() {
  ...
  auto recverThread = std::thread([&]() {
    ...
    std::cout << "[Client] KyberOT+IKNP " << b_t << " ms " << std::endl;
    std::cout << "[Client] KyberOT+IKNP " << (base_end - base_start) / 1000.0
              << " kB" << std::endl;
  });
  ...
  std::cout << "[Server] KyberOT+IKNP " << overall << " ms " << std::endl;
  std::cout << "[Server] KyberOT+IKNP " << (base_end - base_start) / 1000.0
            << " kB " << std::endl;
  recverThread.join();
  return 0;
}
