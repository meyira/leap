...
public:
  ...
  void
  SetTwoIndependentMultiplyShiftParams(std::vector<unsigned __int128> params) {
...
template <typename ItemType, size_t bits_per_item,
          template <size_t> class TableType, typename HashFamily>
std::vector<uint8_t>
CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::serialize()
    const {
...
std::vector<uint8_t>
CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::serialize(
    size_t part_size, size_t start) const {
  static_assert(bits_per_item == 32,
                "serialize only implemented for 32 bit fingerprints");
